...
export default function OperatorsPanel() {
  ...
  const validate = () => {
    ...
    const duplicatePin = operators.some(op => String(op.pin) === trimmedPin && op.id !== form.id);
    if (duplicatePin) {
  ...
  const saveOperator = async () => {
    ...
    setSubmitting(true);
    try {
      const data = { name: form.name.trim(), pin: String(form.pin).trim(), active: form.active };
      if (editMode) {
        await updateDoc(doc(db, "users", form.id), data);
      } else {
        await addDoc(collection(db, "users"), { ...data, role: "operator", createdAt: Date.now() });
      }
      setModalOpen(false);
      await loadOperators();
    } catch (err) {
  ...
  if (loading) return <div className="op-loading">Ładowanie...</div>;

  return (
    <div className="operators-container">
      <div className="operators-header">
        <h2>Operatorzy</h2>
        <button className="btn-add-main" onClick={openAdd}>+ Dodaj</button>
      </div>

      {/* Widok kartowy dla Mobile, Tabela dla Desktop sterowana w CSS */}
      <div className="operators-list">
        {operators.map((op) => (
          <div key={op.id} className={`operator-card ${!op.active ? 'is-blocked' : ''}`}>
            <div className="card-info">
              <span className="op-name">{op.name}</span>
              <span className="op-pin">PIN: <code>{op.pin}</code></span>
              <span className={`status-badge ${op.active ? 'active' : 'blocked'}`}>
            ...
            <div className="card-actions">
              ...
              <button className="btn-edit-small" onClick={() => openEdit(op)}>Edytuj</button>
              <button className="btn-del-small" onClick={() => removeOperator(op)}>Usuń</button>
            </div>
          </div>
        ))}
      </div>
      ...
      {modalOpen && (
        <div className="modal-overlay" onClick={() => !submitting && setModalOpen(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <h3>{editMode ? "Edycja" : "Nowy Operator"}</h3>
            ...
            <div className="modal-btns">
              <button className="save-btn" onClick={saveOperator} disabled={submitting}>
                {submitting ? "..." : "Zapisz"}
              </button>
              <button className="cancel-btn" onClick={() => setModalOpen(false)}>Anuluj</button>
            </div>
          </div>
        </div>
      )}

...
export default function PdfFillMapReport() {
  ...
  // 🔥 Podsumowanie tonażu — sortowane + suma całkowita
  ...
  // 🔥 Generowanie PDF — 2 strony: mapa + tabela
  const generatePDF = async () => {
    ...
    pdf.addImage(mapImg, "PNG", 0, 0, width, height);

    // STRONA 2 — TABELA
    pdf.addPage();
    const tableCanvas = await html2canvas(tableRef.current, { scale: 2 });
    const tableImg = tableCanvas.toDataURL("image/png");
    pdf.addImage(tableImg, "PNG", 0, 0, width, height);
    ...
    <div className="report-box full-width">
      ...
      <div className="report-header">
        ...
        <div className="report-title">
          <h2>Mapa zasypów — Raport PDF</h2>
          <p>
            PZZ Białystok | {new Date().toLocaleDateString()} |{" "}
      ...
      {/* 🔥 STRONA 1 — MAPA */}
      <div ref={mapRef} className="pdf-map-wrapper">
        <Map />
      </div>
